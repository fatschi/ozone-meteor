...
public abstract class SopremoFormat extends ConfigurableSopremoType {
	...
	public void configureForOutput(Configuration configuration, Path path) {
		final Class<? extends SopremoOutputFormat> outputFormat = getOutputFormat();
		if (path != null)
			configuration.setString(FileOutputFormat.FILE_PARAMETER_KEY, path.toUri().toString());
	...
	public void configureForInput(Configuration configuration, Path path) {
		final Class<? extends SopremoInputFormat<?>> inputFormat = getInputFormat();
		if (path != null)
			configuration.setString(FileInputFormat.FILE_PARAMETER_KEY, path.toUri().toString());
